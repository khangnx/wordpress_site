...
if ( ! function_exists( 'top_blog_breadcrumb' ) ) :
	/**
	 * Display Breadcrumb
	 * @return html Breadcrumb with links
	 */
	function top_blog_breadcrumb() {
		...
		$show = top_blog_gtm( 'top_blog_breadcrumb_show' );
		...
		$show_on_home = top_blog_gtm( 'top_blog_breadcrumb_show_home' );

		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'top-blog' ); // text for the 'Home' link
		/* translators: 1: before text/html, 2: after text/html. */
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'top-blog' ); // text for a category page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'top-blog' ); // text for a search results page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'top-blog' ); // text for a tag page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'top-blog' ); // text for an author page
		$text['404']      = esc_html__( '%1$sError 404%2$s', 'top-blog' );; // text for the 404 page
		...
		if ( is_front_page() ) {
			if ( $show_on_home ) {
				echo '
				<div id="breadcrumb">
					<div aria-label="' . esc_attr__( 'Breadcrumbs', 'top-blog' ) . '" class="breadcrumbs breadcrumb-trail on_home">
			...
			echo '
			<div id="breadcrumb">
					<div aria-label="' . esc_attr__( 'Breadcrumbs', 'top-blog' ) . '" class="breadcrumbs breadcrumb-trail">
						<ol itemscope itemtype="http://schema.org/BreadcrumbList" class="trail-items">';
			...
			if ( is_home() ) {
				...
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'top-blog' ) ) ) ), esc_html( get_the_time( __( 'Y', 'top-blog' ) ) ), $content_position++ ) ;

				echo sprintf( $link, esc_url( get_month_link( get_the_time( __( 'Y', 'top-blog' ) ) ) ), esc_html(get_the_time( __( 'F', 'top-blog' ) ) ), $content_position++ );

				echo $before . esc_html( get_the_time( __( 'd', 'top-blog' ) ) ) . sprintf( $after, $content_position++ );
			} elseif ( is_month() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'top-blog' ) ) ) ), esc_html( get_the_time( __( 'Y', 'top-blog' ) ) ), $content_position++ ) ;

				echo $before . esc_html( get_the_time( __( 'F', 'top-blog' ) ) ) . sprintf( $after, $content_position++ );
			} elseif ( is_year() ) {
				echo $before . esc_html( get_the_time( __( 'Y', 'top-blog' ) ) ) . sprintf( $after, $content_position++ );
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'top-blog' ), absint( max( $paged, $page ) ) );

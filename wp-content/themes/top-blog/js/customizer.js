...
jQuery( document ).ready(function($) {
	 ...
	 */
	// Init sortable.
	$('#sub-accordion-panel-top_blog_sp_sortable').sortable( {
		items : "> li:gt(0)",
		axis : 'y',
		helper : 'clone',
		...
		update: function( event, ui ) {
			...
			$.each( inputValues, function( index, value ) {
				inputValues[index] = inputValues[index].replace('accordion-section-top_blog_ss_','');
			});
			...
			$( '#_customize-input-top_blog_ss_order' ).val( inputValues ).trigger('change');
	...
	$('#accordion-section-top_blog_ss_main_content h3').off("click");
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					topBlogAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$(this).find('.sortable_repeater.sortable').sortable({
		update: function(event, ui) {
			topBlogGetAllInputs($(this).parent());
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				topBlogGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			topBlogGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	function topBlogAppendRow($element, defaultValue) {
	...
	function topBlogGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  topBlogGetAllImageCheckboxes($(this).parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function topBlogGetAllImageCheckboxes($element) {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].topBlogtinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].topBlogtinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].topBlogmediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['top-blog-upsell'] = api.Section.extend( {
